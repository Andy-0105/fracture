...
def convertCocoSegmentToMask(coco_path, mask_save_path):
    with open(coco_path, 'r', encoding='utf-8') as coco_f:
        ...
        for coco_image in coco_images:
            ...
            coco_filename = coco_image_cache[f"{coco_image['id']}"]["file_name"]
            coco_zero_path=os.path.join(mask_save_path, coco_filename)
            cv2.imwrite(coco_zero_path[:-3] + 'png', mask_image)

        coco_annotations = coco_dict['annotations']
        for coco_annotation in coco_annotations:
            coco_filename = coco_image_cache[f"{coco_annotation['image_id']}"]["file_name"]
            coco_mask_path = os.path.join(mask_save_path, coco_filename[:-3] + 'png')
            ...
            cv2.imwrite(coco_mask_path[:-3] + 'png', mask_image)

def main():
    coco_path = os.path.join(os.getcwd(), './json/new48time/4.json')
    mask_save_path = os.path.join(os.getcwd(), './demo/label/new48time/4')
